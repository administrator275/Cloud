...
const C = {
  ...
  muted: "#5A7A85", gold: "#E8A020", red: "#D94F3D",
};
...
// ── Stable top-level components (never redefined inside render) ──
...
function AddStaffForm({ onAdd }) {
  ...
  const handleAdd = useCallback(async () => {
    ...
    await onAdd({ name: name.trim(), salary: parseFloat(salary), doj: doj || getTodayStr() });
    setName(""); setSalary(""); setDoj("");
  }, [name, salary, doj, onAdd]);
...
export default function App() {
  ...
  function getStaffLeaves(staffId, year) {
    const approved = leaves.filter(l => l.staffId === staffId && l.status === "Approved" && new Date(l.from).getFullYear() === year);
    const summary = {};
  ...
  function getPayroll(staffId, month, year) {
    ...
    if (!st) return { gross: 0, lopDays: 0, deduction: 0, net: 0, totalLeaveDays: 0 };
    const monthLeaves = leaves.filter(l => l.staffId === staffId && l.status === "Approved" && new Date(l.from).getMonth() === month && new Date(l.from).getFullYear() === year);
    ...
    const deduction = Math.round((lopDays / daysInMonth) * st.salary);
    return { gross: st.salary, lopDays, deduction, net: st.salary - deduction, totalLeaveDays };
  }
  ...
  return (
    <div style={s.app}>
      ...
      {/* Admin View */}
      {view === "admin" && !adminUnlocked && <AdminPin onUnlock={handleUnlock} />}

      {view === "admin" && adminUnlocked && (
        <div style={s.wrap}>
          {/* Stats */}
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "14px", marginBottom: "18px" }}>
            <div style={s.statCard(C.gold)}><p style={{ fontSize: "10px", color: C.muted, textTransform: "uppercase", letterSpacing: "1px", margin: "0 0 4px" }}>Pending</p><p style={{ fontSize: "26px", fontWeight: "800", color: C.gold, margin: 0 }}>{pendingCount}</p></div>
                  ...
                  return (
                    <div key={l.id} style={{ ...s.row, alignItems: "flex-start" }}>
                      ...
                      <div style={{ display: "flex", gap: "6px", alignItems: "center", flexWrap: "wrap" }}>
                        ...
                        {l.status === "Pending" && <>
                          <button style={{ ...s.btn("#16a34a"), padding: "5px 10px", fontSize: "11px" }} onClick={() => updateLeaveStatus(l.id, "Approved")}>✓</button>
                          <button style={{ ...s.btn(C.red), padding: "5px 10px", fontSize: "11px" }} onClick={() => updateLeaveStatus(l.id, "Rejected")}>✗</button>
                        </>}
            ...
            <>
              ...
              <div style={s.card}>
                <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "14px" }}>
                  ...
                  <input type="number" style={{ ...s.input, width: "90px" }} value={filterYear} onChange={e => setFilterYear(Number(e.target.value))} inputMode="numeric" />
                </div>
              ...
              {staff.length === 0 ? <p style={{ color: C.muted }}>No staff yet.</p> : (
                <div style={{ overflowX: "auto" }}>
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px" }}>
                    <thead>
                      <tr>{["Name","Gross (₹)","Leave Days","LOP Days","Deduction (₹)","Net Pay (₹)"].map(h => (
